...
public class MainActivity extends AppCompatActivity {
     ...
     * find the Input box based on its ID. We also add a listener to the input text box.
     ...
     * We use this information to see if we should enable or disable our buttons).
     */
     ...
     * and we initialize the gridButtons array.
     */
     ...
     * (add a String 'operator' + 'operand' in the manager), and
     * we update the grid to reflect this new addition
    ...
    public void eqClick(View view) {
        ...
        // Get the user-inputted number and set the Calculator
     ...
     * <p>
     * First, we find the operator chosen by the user by analyzing the text of the button.
     ...
     * (with the getText() method and call .toString() on that sequence.
     *
     ...
     * and set the Calculator's variables to the ones found in the button - if we are trying to
     * find the opposite operation, we use the static method oppositeOperator(String) from
     * the Calculator class - and then apply the operation.
     *
     ...
     * This method calls the canEnableEquals() method to check if we fulfill the requirements
     ...
     * We calculate this by seeing if there is a parsable double in the operand text box
     ...
     *      <p>we <b>disable</b> <u>both</u> buttons
     * <p>
     * If we are not at the last index:
     *      <p>we <b>enable</b> the <u>Undo</u> button
     * <p>
     * If we are not at index 0:
     *      <p>we <b>enable</b> the <u>Redo</u> button
     * <p>
     * Similarly to the updateEqButton() method, the actual conditions are
     * split up into the helper methods canEnableUndo() and canEnableRedo().
     */
     ...
     * necessary size manipulations on the text (with the fixTextSize() method).
     *
     ...
     *
     * Since numbers that are too large occasionally break the GridView, we change the text size
     * for larger numbers.
     *
